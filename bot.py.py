...
# 🔄 تحديث المكتبات الأساسية تلقائياً فور تشغيل البوت
print("🔄 جاري التأكد من تحديث المكتبات...")
try:
    subprocess.check_call([sys.executable, "-m", "pip", "install", "--upgrade", "yt-dlp", "imageio-ffmpeg"])
    print("✅ تم تحديث المكتبات بنجاح!")
except Exception as e:
    print(f"⚠️ خطأ أثناء التحديث التلقائي: {e}")
...
print(f"🎬 مسار FFmpeg الحالي: {FFMPEG_PATH}")
...
COMMON_YDL_OPTS = {
    'quiet': True,
    'no_warnings': True,
    'ffmpeg_location': FFMPEG_PATH,  # 👈 تمرير مسار FFmpeg المباشر هنا حل المشكلة تماماً!
    'http_headers': {
...
def clean_url(url: str) -> str:
    if "tiktok.com" in url and "?" in url and not ("vt.tiktok" in url or "vm.tiktok" in url):
        return url.split('?')[0]
    return url
...
def main():
    ...
    print("🚀 البوت يعمل الآن بشكل كامل بدون أي مشاكل في FFmpeg...")
    app.run_polling()
